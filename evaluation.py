...
from utils.label_lookup import get_label_lookup_from_first_token, combine_slot_values, get_label_lookup
...
def main(args):
    ...
    onlyslots = processor.recovered_onlyslots
    ...
    domain_lookup = get_label_lookup(domains, tokenizer, sv_encoder, device)
    onlyslots_lookup = get_label_lookup_from_first_token(onlyslots, tokenizer, sv_encoder, device)
    
    # load state_dict
    ckpt_path = os.path.join(args.save_dir, 'model_best_acc.bin')
    model = BeliefTracker(args, slot_lookup, value_lookup, domain_lookup, onlyslots_lookup, num_labels, slot_value_pos, device)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_dir", default='out-bert/7558', type=str)
    parser.add_argument("--attn_type", default='softmax', type=str)
    parser.add_argument("--dropout_prob", default=0.1, type=float)

    parser.add_argument("--max_seq_length", default=512, type=int)
    parser.add_argument("--attn_head", default=16, type=int)
    parser.add_argument("--attn_rules", default=32, type=int)
    parser.add_argument("--attn_qk_dim", default=768, type=int)
