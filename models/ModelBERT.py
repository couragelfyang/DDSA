...
class DisentangledAttention(nn.Module):
    ...
    def forward(self, domain_q, onlyslot_q, k, v):
        bsz = domain_q.size(0)
...
class UtteranceAttention(nn.Module):
    def __init__(self, attn_head, attn_rules, attn_qk_dim, model_output_dim, dropout=0., attn_type="softmax"):
        ...
        self.attn_fun = DisentangledAttention(self.model_output_dim, self.attn_head, self.attn_rules, self.attn_qk_dim)
    ...
    def forward(self, slot_query, domain_query, onlyslot_query, value, attention_mask=None):
        ...
        attended_embedding = self.attn_fun(expanded_domain_query, expanded_onlyslot_query, new_value, new_value)
